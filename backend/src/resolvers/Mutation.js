...
const Mutations = {
  ...
  async signup(parent, args, ctx, info) {
    ...
    // setting the jwt as a cookie on the response
  ...
  async resetPassword(parent, args, ctx, info) {
    ...
    if (!user) {
      throw new Error("This token is either envalid or expired");
    }
    // hash the new password
    const password = await bcrypt.hash(args.password, 10);
    // save the new password, removing old resetToken
  ...
  async addToCart(parent, args, ctx, info) {
    ...
    // if not, create a new CartItem for user
  ...
  async createOrder(parent, args, ctx, info) {
    // 1. query the current user, make sure that they are sign in
    const { userId } = ctx.request;
    if (!userId)
    ...
    // 6. clean the user's cart, delete cartItems
